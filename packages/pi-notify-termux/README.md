...
| # | 权限 | 怎么配 | 影响什么 |
|---|------|--------|----------|
| 1 | **Termux:API 通知权限** | 设置 → 应用 → Termux:API → 通知 → **全部开启** | 通知发送 / 移除。**ColorOS/OPPO 必须开全**：通知权限有细分类别，只开总开关会导致 `cancel()`（移除通知）被系统静默忽略——通知发得出去但永远不消失 |
| 2 | **Termux:API 通知使用权（监听服务）** | 设置 → 搜「通知使用权」→ 开启 Termux:API（原生在 设置→通知→高级；ColorOS 在 设置→通知与状态栏→通知管理→高级设置） | **通知栏列表 / 权限自检**：`termux-notification-list` 依赖 NotificationListenerService，与通知权限是**两个独立开关**。未开启或服务被系统杀掉时列表恒为空，自检误报“权限未开全”。**开关已开仍失效 → 先关再开强制重绑，仍不行重启手机** |
| 3 | **Termux 通知权限** | 设置 → 应用 → Termux → 通知 → 允许 | 通知按钮 action 经 Termux 服务执行；Termux 常驻通知显示任务数（"Tasks: N"为正常现象） |
| 4 | **Termux "后台弹出界面"（仅 ColorOS/OPPO）** | 设置 → 应用管理 → Termux → 权限 → 后台弹出界面 → 允许 | **"打开终端"按钮**。Android 10+ 禁止后台 app 启动 Activity；不开则后台点按钮切不回 Termux（已降级为 Toast 提示）。改后通常无需重启，如无效请强制停止 Termux 再开 |
| 5 | 电池优化白名单（建议） | 设置 → 电池 → 后台耗电管理 → Termux 允许 | 长任务 + 通知链路不被杀；锁屏后通知照常弹出 |

Android 13+ 首次使用后可用 `/notify` 查看状态提示。

**权限自动检测**：扩展启动时与 `/notify` 时自动执行静默自检（发 `--alert-once` 诊断通知 → `termux-notification-list` 确认在栏 → 立即移除）；检测失败会弹出警告——**同时检查①②两项**：通知权限全开 + 通知使用权（监听服务）开启。后者被系统杀掉时列表同样读不到，会误报“权限未开全”（先关再开强制重绑，或重启手机）。
