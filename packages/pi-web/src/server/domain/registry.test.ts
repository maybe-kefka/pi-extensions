...
describe("parseStateFile / serializeStateFile", () => {
  it("解析合法状态文件", () => {
    ...
    expect(state).toEqual({ port: 3939, token: "abc12345def", hostPid: 123, startedAt: 1000 });
  });
  ...
  it("非法输入返回 null（坏 JSON / 缺字段 / token 过短）", () => {
    ...
    expect(parseStateFile(JSON.stringify({ port: 3939, token: "short", hostPid: 1 }))).toBeNull();
    expect(parseStateFile(JSON.stringify({ token: "abcdefgh", hostPid: 1 }))).toBeNull();
  });

  it("serialize 往返一致；缺 startedAt 自动补当前时间", () => {
    const state = parseStateFile(serializeStateFile({ port: 8080, token: "toktoktoktok", hostPid: 2, startedAt: 5 }));
    expect(state).toEqual({ port: 8080, token: "toktoktoktok", hostPid: 2, startedAt: 5 });
  });
