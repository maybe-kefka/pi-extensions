...
describe("computeContextBreakdown", () => {
	it("computes five categories, conversation split and ratios", () => {
		const input: ContextBreakdownInput = {
			...
			contextFiles: [{ path: "AGENTS.md", content: "dddddddd" }], // 16 -> 4
		...
		// system = 2 + 1 + 1 = 4
		expect(result.categories.find((c) => c.key === "system")?.tokens).toBe(4);
		expect(result.categories.find((c) => c.key === "contextFiles")?.tokens).toBe(5); // "AGENTS.md\ndddddddd" = 17 chars
